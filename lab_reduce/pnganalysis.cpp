	...
	#pragma omp parallel
	{
		...
		for (int i = 0; i < image.width(); i++) {
			...
			for (int j = 0; j < image.height(); j++) {
				...
				if ( select(*image(i,j)) ) local_selected++;
...
map<RGBAPixel, int> pnganalysis::frequency( const PNG & image ) {
	...
	
	{
		...
		map<RGBAPixel, int> local_ret_freq;
		...
		for( int i = 0; i < image.width(); ++i ) {
			...
			for( int j = 0, height = image.height(); j < height; ++j ) {
				...
				++local_ret_freq[ *image(i,j) ];
			}
		}
		
		#pragma omp critical
		
		for (map<RGBAPixel, int>::iterator k = local_ret_freq.begin(); k != local_ret_freq.end(); k++) {
		
			ret_freq[k->first] += k->second;
