...
void Collage::drawByCoordinatesSerial(PNG * result)
{
   ...
   for(int i = 0; i < width; i++) {
		...
		while ((r >= 0) && (!finished)){
			...
			int offsetW = coords[r].x;
			
        		int offsetH = coords[r].y;
        		
			int e_x = layers[r].width() + offsetW;
			
			int e_y = layers[r].height() + offsetH;
			
			if (i >= offsetW && i < e_x && j >= offsetH && (j < e_y)) {
				
					*(*result)(i, j) = *(layers[r](i-offsetW, j-offsetH));
					 finished = true;
			}
			else r--;
		}
