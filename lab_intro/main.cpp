...
PNG brighten(PNG original, int amount)
{
    ...
    for (int yi = 0; yi < original.height(); yi++)
    {
		...
		original(xi, yi)->red = ((red + amount) < 255 ? (red + amount) : 255);
		original(xi, yi)->green = ((green + amount) < 255 ? (green + amount) : 255);
		original(xi, yi)->blue = ((blue + amount) < 255 ? (blue + amount) : 255);
...
for (int yi = 0; yi < firstImage.height(); yi++)
    {
        ...
        {	
		int newRed = ((firstImage(xi, yi)->red) + (secondImage(xi, yi)->red))/2;
		int newGreen = ((firstImage(xi, yi)->green) + (secondImage(xi, yi)->green))/2;
		int newBlue = ((firstImage(xi, yi)->blue) + (secondImage(xi, yi)->blue))/2;
		
		firstImage(xi, yi)->red = newRed;
		firstImage(xi, yi)->green = newGreen;
		firstImage(xi, yi)->blue = newBlue;
